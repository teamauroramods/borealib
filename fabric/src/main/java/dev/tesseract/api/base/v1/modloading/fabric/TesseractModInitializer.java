package dev.tesseract.api.base.v1.modloading.fabric;
...
import dev.tesseract.impl.base.platform.fabric.ModContainerImpl;
...
public interface TesseractModInitializer extends ModInitializer, DataGeneratorEntrypoint {
    ...
    default void onInitialize() {
        ModLoaderService s = Tesseract.findMod(this.id());
    ...
    default void onInitializeDataGenerator(FabricDataGenerator generator) {
        Tesseract.findMod(this.id()).onDataInit(new ModLoaderService.DataGeneratorContext() {
    ...
    default void buildRegistry(RegistrySetBuilder builder) {
        Tesseract.findMod(this.id()).buildRegistries(builder);
    }
}
