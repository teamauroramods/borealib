...
public class FabricConfigSpec extends UnmodifiableConfigWrapper<UnmodifiableConfig> {
    ...
    public synchronized boolean isCorrect(CommentedConfig config) {
        ...
        return correct(this.config, config, null, parentPath, Collections.unmodifiableList(parentPath), (a, b, c, d) -> {
        }, null, true) == 0;
    }
    ...
    public static class ValueSpec {
        ...
        ValueSpec(Supplier<?> supplier, Predicate<Object> validator, ConfigBuilderImpl.BuilderContext context) {
    ...
    public static class FabricConfigValue<T> implements ConfigValue<T> {
        @VisibleForTesting
        static boolean USE_CACHES = true;

        private final ConfigBuilderImpl parent;
        private final List<String> path;
        private final Supplier<T> defaultSupplier;
        FabricConfigSpec spec;
        private T cachedValue = null;

        FabricConfigValue(ConfigBuilderImpl parent, List<String> path, Supplier<T> defaultSupplier) {
            this.parent = parent;
            this.path = path;
    ...
    public static class BooleanValue extends FabricConfigValue<Boolean> {
        BooleanValue(ConfigBuilderImpl parent, List<String> path, Supplier<Boolean> defaultSupplier) {
            super(parent, path, defaultSupplier);
        }
    }

    public static class IntValue extends FabricConfigValue<Integer> {
        IntValue(ConfigBuilderImpl parent, List<String> path, Supplier<Integer> defaultSupplier) {
            super(parent, path, defaultSupplier);
        }
    ...
    public static class LongValue extends FabricConfigValue<Long> {
        LongValue(ConfigBuilderImpl parent, List<String> path, Supplier<Long> defaultSupplier) {
            super(parent, path, defaultSupplier);
        }
    ...
    public static class DoubleValue extends FabricConfigValue<Double> {
        DoubleValue(ConfigBuilderImpl parent, List<String> path, Supplier<Double> defaultSupplier) {
            super(parent, path, defaultSupplier);
        }
    ...
    public static class EnumValue<T extends Enum<T>> extends FabricConfigValue<T> {
        ...
        EnumValue(ConfigBuilderImpl parent, List<String> path, Supplier<T> defaultSupplier, EnumGetMethod converter, Class<T> clazz) {
            super(parent, path, defaultSupplier);
