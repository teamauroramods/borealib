...
public class CustomBoat extends Boat {
    ...
    public CustomBoat(Level level, double d, double e, double f) {
        this(MagnetosphereEntityTypes.BOAT.get(), level);
        this.setPos(d, e, f);
    ...
    protected void addAdditionalSaveData(CompoundTag compound) {
        ResourceLocation id = MagnetosphereRegistries.BOAT_TYPES.getKey(this.getBoatCustomType());
        if (id != null)
    ...
    protected void readAdditionalSaveData(CompoundTag compound) {
        if (compound.contains("Type", NBTConstants.STRING))
            this.setPollenType(MagnetosphereRegistries.BOAT_TYPES.get(new ResourceLocation(compound.getString("Type"))));
    }

    public void setPollenType(@Nullable CustomBoatType boatType) {
        this.entityData.set(DATA_ID_TYPE, boatType == null ? -1 : MagnetosphereRegistries.BOAT_TYPES.getId(boatType));
    }

    public CustomBoatType getBoatCustomType() {
        int id = this.entityData.get(DATA_ID_TYPE);
        return id == -1 ? null : MagnetosphereRegistries.BOAT_TYPES.byId(id);
    }
}